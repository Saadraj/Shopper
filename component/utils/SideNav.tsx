...
const SideBar = () => {
    ...
    const subHandleChange = (panel: string) => (
        ...
        <Box className={classes.root}>
            <Container maxWidth="xl">
                <Grid container>
                                ...
                                item?.list?.length && (
                                    <Grid item xs={12} key={item.title}>
                                        <FormControlLabel
                                            ...
                                            checked={Boolean(Math.round((Math.random() * 10) / 10))}
                    ...
                    <Grid container item xs={12} className={classes.commonPadding}>
                        {menu.map(
                            (item) =>
                                item?.list?.length && (
                                    <Accordion
                                            ...
                                            <Grid container>
                                                {item.list?.map((p) => (
                                                    <Grid item xs={12} key={p}>
                                                        <Accordion
                                                            expanded={subExpanded === p}
                                                            onChange={subHandleChange(p)}
                                                        >
                                                            <AccordionSummary
                                                                expandIcon={<ExpandMoreIcon />}
                                                                aria-controls="panel1a-content"
